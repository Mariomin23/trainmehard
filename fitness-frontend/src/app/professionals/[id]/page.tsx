...
export default function ProfessionalDetailPage() {
  ...
  const priceForDuration = +((professional.hourlyRate * duration) / 60).toFixed(2);
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-100">
              <div className="flex items-start gap-5 mb-6">
                <div className="w-20 h-20 rounded-2xl bg-green-100 flex items-center justify-center text-green-700 font-bold text-3xl shrink-0">
                  {professional.userId.name.charAt(0)}
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">{professional.userId.name}</h1>
                  <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
                    <Star size={14} className="text-yellow-400 fill-yellow-400" />
                    {professional.rating > 0 ? professional.rating.toFixed(1) : 'Nuevo profesional'}
                    {professional.reviewsCount > 0 && <span>· {professional.reviewsCount} reseñas</span>}
                  </div>
          ...
          <div className="md:col-span-1">
            <div className="bg-white rounded-3xl p-6 border border-gray-100 sticky top-24">
              <div className="text-center mb-6">
                <p className="text-4xl font-bold text-gray-900">{professional.hourlyRate}€</p>
